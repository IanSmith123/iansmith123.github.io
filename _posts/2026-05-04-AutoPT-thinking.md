...
尽管如此，我们还是准备了一系列的RAG库供大模型按需调用，RAG是个体力活，大家的设计都大差不差，此处按下不表（懒得写了）。
...
kali 软件源仓库存在供应链攻击的风险脚下，风险较大的是调用的一些 Golang 的第三方库（Third-Party libraries，TPL），但基本都是1000星星以上的GitHub 项目，如果出现供应链攻击了那估计又是一个大新闻。
...
# 线下决赛见闻
不得不说主办方的李鑫是个很有趣的人，把决赛选在了夜店。一群黑客坐在下面看大神在舞台上讲Agent设计，讲最新攻击思路，偶尔还有歌手上去一展歌喉，属实有趣，很符合外界对黑客洒脱自由的定位（我是去打酱油的，算不上黑客）。
...
在两年前我跟同学聊天还说全自动漏洞利用距离我们还很遥远，但这梦幻的场景我们真的看到了，而且亲手实现的，未来真的来了。

回头想想，调用大模型API算什么大模型研究，增量都是大模型本身带来的。我们只是围绕某个目标，为大模型设计了一点点提示罢了 :)
