...
比赛前几天看主办方的群里有中转站的广告，很早以前也听人说claude +Opus 4.6在渗透测试处于无敌状态，看得心里痒，于是我们也加入了Opus 4.6的阵营。

在实际测试Opus 4.6能力之前，我一直以为Agent的总体能力一半取决于harness设计，另外一半是模型能力。然而当我用Opus 4.6做了个多层内网渗透的靶场之后，我被震撼的说不出话，Opus 4.6在7分钟内完成了DeepSeek V3.2两个小时的进度。
...
但是，在我用了Opus 4.6之后这个想法发生了改变，基模记忆足够好的情况下，可能RAG的增量会没那么明显，反而可能会因为过多杂乱信息注入分散模型注意力。DeepSeek参数685B，激活量37B，而传言中Opus 4.6参数大约2T，激活量超过100B。参数量带来的模型能力提升是肉眼可见的。

尽管如此，我们还是准备了一系列的RAG数据供大模型按需调用，RAG是一个复杂的工程，有好产品队伍的应该不太会拱手送给主办方，大家比赛中用到的设计估计都差不太多。

在单机/无GPU/磁盘空间局促这三个限制下，做高质量的RAG堪比螺蛳壳里做道场。我们融入了技战术、CVE漏洞描述、指纹匹配、EXP库等等内容，具体方案细节是大家在互联网上都能检索到的混合检索那一套，此处按下不表。
...
# 线下决赛见闻
不得不说主办方的李鑫是个很有趣的人，把决赛选在了live house。一群黑客坐在下面看大神在舞台上讲Agent设计，讲最新攻击思路，偶尔还有歌手上去一展歌喉，属实有趣，很符合外界对黑客洒脱自由的定位（我是去打酱油的，算不上黑客）。
...
LLM中转站注入恶意指令的攻击面影响还是太大了，即使管理员没有恶意的想法，但是如果中转站框架，如new-api本身存在漏洞，那么依然存在巨大的攻击面，让所有的用户执行恶意的tool也就不难实现了，再往上可以是模型原始供应商返回结果被注入恶意指令，那么影响所有的大模型用户。上个月new-api是stripe 空密钥导致的恶意充值漏洞，下一次可能就是rce了，这种供应链攻击风险无处不在，毕竟我们这也搞供应链漏洞分析 :)

很多队伍同时用多个厂商的大模型并行做题，我们还是太保守了，只做了agent并发，没做api并发。这要做个两两组合那资金消耗不敢想啊 :)
